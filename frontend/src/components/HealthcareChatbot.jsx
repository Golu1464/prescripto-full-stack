...
`.trim();
      ...
      const urlWithKey = endpoint + (geminiApiKey ? `?key=${encodeURIComponent(geminiApiKey)}` : '');
      ...
      const res = await fetch(urlWithKey, {
        ...
        headers: {
          ...
          ...(geminiApiKey ? { 'x-goog-api-key': geminiApiKey } : {})
        },
      ...
      if (!res.ok) {
        ...
        const userFriendly = res.status === 401 || res.status === 403
          ? `API key error (${res.status}). Check key & referrer restrictions in Google Cloud Console.`
